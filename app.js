...
function bindEvents() {
  ...
  els.imageInputMobileProxy?.addEventListener('change', handleImageUpload);
  ...
  els.mobileTextCardBtn?.addEventListener('click', () => openModal(els.textCardModal));
  ...
  els.mobileDownloadBtn?.addEventListener('click', downloadCanvas);
  ...
  if (els.imageTextPreview) bindImageTextCanvas();
}
...
function syncSpacingControls() {
  ...
  if (els.gapValue) els.gapValue.textContent = `${rowGap} px`;
  if (els.columnGapValue) els.columnGapValue.textContent = `${colGap} px`;
  renderKanban();
  throttledDrawCanvas();
  triggerAutoSave();
}
...
function setWhiteBorder(on) {
  if (!els.whiteBorderToggle || !els.whiteBorderText) return;
...
function getWhiteBorderEnabled() {
  return els.whiteBorderToggle?.classList.contains('toggle-on') || false;
}
...
function refreshFilename() {
  ...
  if (els.filenameInput) els.filenameInput.value = currentFilename;
}

function initColumnsForLayout(layout, preserve = false) {
  const prevItems = preserve ? columnsState.flatMap(c => c.items) : [];
  const count = layout === 'special_2_1' ? 3 : Number(layout || 3);
  ...
  prevItems.forEach((item, idx) => {
    newCols[idx % newCols.length].items.push(item);
  });
        ...
        <div class="kanban-card-frame">
          ...
          <div class="kanban-drag-content">
            <div class="kanban-thumb-shell">
              <img class="kanban-thumb" src="${reg.previewData || reg.originalData}" alt="thumb" loading="lazy" decoding="async">
            </div>
...
function getSettings() {
  return {
    layoutMode: els.layoutMode?.value || '3',
    defaultGap: getEffectiveRowGap(els.defaultGap?.value),
    columnGap: getEffectiveColumnGap(getColumnGapValue()),
    frameStyle: els.frameStyle?.value || 'editorial-luxe',
    globalBgColor: els.globalBgColor?.value || '#f8fafc',
    innerBgColor: els.innerBgColor?.value || '#ffffff',
    patternColor: els.patternColor?.value || '#c9a227',
...
function drawSpecialLayout(ctx, settings, safeX, safeY, safeW, safeH) {
  ...
  const baseBottomWidth = Math.min(
    safeW * 0.78,
    Math.max(baseTopColWidth * 1.18, baseTopColWidth * 1.34)
  );
...
function drawMultiLineTextOnCanvas(ctx, text, x, y, maxWidth, lineHeight, alignH = 'center', alignV = 'center') {
  ...
  if (alignV === 'center') startY = y - totalHeight / 2 + lineHeight * 0.8;
...
function measureTextCardLayout() {
  ...
  const fontSize = Number(els.textCardFontSize?.value || 52);
  const lineHeight = fontSize * 1.45;
  const measureCanvas = document.createElement('canvas');
  const mctx = measureCanvas.getContext('2d');
  mctx.font = `600 ${fontSize}px sans-serif`;
  const maxWidth = 760;
  const result = drawMultiLineTextOnCanvas(mctx, text, 0, 0, maxWidth, lineHeight, els.textCardAlignH?.value || 'center', 'top');
...
function drawTextCardPreview() {
  if (!els.textCardPreview) return null;
  ...
  ctx.fillStyle = els.textCardBgColor?.value || '#ffffff';
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  ctx.fillStyle = els.textCardTextColor?.value || '#0f172a';
  ctx.font = `600 ${layout.fontSize}px sans-serif`;

  const alignH = els.textCardAlignH?.value || 'center';
  const alignV = els.textCardAlignV?.value || 'center';

  const anchorX =
    alignH === 'left' ? layout.paddingX :
    alignH === 'right' ? canvas.width - layout.paddingX :
    canvas.width / 2;

  const anchorY =
    alignV === 'top' ? layout.paddingY :
    alignV === 'bottom' ? canvas.height - layout.paddingY :
    canvas.height / 2;
...
async function openImageTextEditor(id) {
  ...
  if (!reg || !els.imageTextPreview) return;
...
function syncControlsFromTextState() {
  if (els.imageTextContent) els.imageTextContent.value = textState.text;
  if (els.imageTextColor) els.imageTextColor.value = textState.color;
  if (els.imageTextSize) els.imageTextSize.value = textState.fontSizeRatio;
  if (els.imageTextAlign) els.imageTextAlign.value = textState.alignH;
}

function syncImageTextControls() {
  textState.text = els.imageTextContent?.value || '';
  textState.color = els.imageTextColor?.value || '#ffffff';
  textState.fontSizeRatio = Number(els.imageTextSize?.value || 7);
  textState.alignH = els.imageTextAlign?.value || 'center';
  drawImageTextPreview();
}
...
function drawImageTextPreview() {
  ...
  if (previewBaseImage) {
    ctx.drawImage(previewBaseImage, 0, 0, c.width, c.height);
  }
...
async function applyImageText() {
  if (!activeImageEditId || !previewBaseImage || !els.imageTextPreview) return;
...
function updateSaveStatus(state) {
  if (!els.saveDot || !els.saveText) return;
  const map = {
...
function buildWorkspacePayload() {
  ...
  Object.entries(imageRegistry).forEach(([id, item]) => {
    images[id] = {
      previewData: item.previewData || item.originalData,
...
async function loadWorkspace() {
  ...
  try {
    ...
    if (els.frameStyle) els.frameStyle.value = workspace.settings?.frameStyle || 'editorial-luxe';
    if (els.globalBgColor) els.globalBgColor.value = workspace.settings?.globalBgColor || '#f8fafc';
    if (els.innerBgColor) els.innerBgColor.value = workspace.settings?.innerBgColor || '#ffffff';
    if (els.patternColor) els.patternColor.value = workspace.settings?.patternColor || '#c9a227';

    updateSwatchSelection('globalBgColor', els.globalBgColor?.value || '#f8fafc');
    updateSwatchSelection('innerBgColor', els.innerBgColor?.value || '#ffffff');
...
async function clearAll() {
  ...
  try {
    ...
    if (els.layoutMode) els.layoutMode.value = '3';
    initColumnsForLayout('3');

    if (els.defaultGap) els.defaultGap.value = 12;
    if (els.columnGap) els.columnGap.value = 24;
    syncSpacingControls();

    if (els.frameStyle) els.frameStyle.value = 'editorial-luxe';
    if (els.globalBgColor) els.globalBgColor.value = '#f8fafc';
    if (els.innerBgColor) els.innerBgColor.value = '#ffffff';
    if (els.patternColor) els.patternColor.value = '#c9a227';
    ...
    if (els.textCardContent) els.textCardContent.value = '';
    if (els.textCardTextColor) els.textCardTextColor.value = '#0f172a';
    if (els.textCardBgColor) els.textCardBgColor.value = '#ffffff';
    if (els.textCardFontSize) els.textCardFontSize.value = 52;
    if (els.textCardAlignH) els.textCardAlignH.value = 'center';
    if (els.textCardAlignV) els.textCardAlignV.value = 'center';
